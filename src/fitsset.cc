...
while (!list.eof())
  {
	...
	if (dbimage.IsValid())
	  {
	    string raw = dbimage.FitsImageName(Raw);
	    string cal = dbimage.FitsImageName(Calibrated);
	    if (FileExists(raw))
	      {
		fileName = raw;
	      }
	    else if (FileExists(cal))
	      {
		fileName = cal;
	      }
	    else
	      {
		cout << "FitsSet : Image " << fileName << " not found neither in the working directory nor in the database. "<< endl; 
		fileName = "";
	      }
	  }
	else
