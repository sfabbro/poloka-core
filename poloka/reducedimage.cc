			  ...
			  bool use_sigma_header)
{
  ...
  */

  {
    ...
    if (IsOfKind<CfhtMegacam>(head) && Saturation()< 50000 && HasFlat())
      {
...
bool ReducedImage::MakeSatur()
{
  ...
  if (cards.HasKey("SATUR_SEP_AMPS") && cards.IParam("SATUR_SEP_AMPS") == 1)
    {
      ...
      for (int iamp=1; iamp<= namp; ++iamp)
	{
	  ...
	  double saturamp = ComputeSaturation(subimage);
	  if (satur > saturamp) satur = saturamp;
	}
    }
  else
