...
double ComputeDrift(const Image& Im) {
  double minmedian = 1000000.;
  double maxmedian = 0.;
  const double maxall = Im.MaxValue();
  // block loop
  // we'd rather take larger blocks than smaller ones
  // => when near the edge, stretch the block a little bit
  bool xbreak = false;
  int x0 = 0;
  int x1 = 64;
  while (true) {
    ...
    int y1 = 64;
    while (true) {
      double median = FArrayMedian(Im.begin(), Im.Nx()*Im.Ny());
