...
C
     ...
     .     PERERR, PROERR, SKYIN, SKYOUT, ISTAT, GLOBAL_SKY)
C
C=======================================================================
C
...
C
      INTEGER NCOL, NROW
      CHARACTER*256 COOFIL, MAGFIL, PSFFIL, PROFIL, GRPFIL, SWITCH
      CHARACTER LINE*80, SUBPIC*256, EXTEND*256, CASE*5
...
C
C  960 CALL GETNAM ('Input file:', MAGFIL)
...
C  970 CALL GETNAM ('File for results:', PROFIL)
...
C      IF (SUBPIC(1:11) .EQ. 'END-OF-FILE') CALL TBLANK
C
...
C
 2197       IF (N .GT. MINSKY) THEN
               ...
               DF = 4.*(RONOIS + AMAX1(0.5,D+GLOBAL_SKY)/PHPADU)
               DX = 0.
               DY = 0.
               DO I=1,N
                  DPOS = NUMER(I) - D
...
C
   ...
   61       FORMAT ('Group too large:', I6, '  (', F5.2, ')')
...
C
   ...
   63             FORMAT (I6, 2F9.3, F9.3, 9X, A, F6.3)
               END IF
...
C
      SIGSQ = SIGMA(IX,IY) + (DPOS+GLOBAL_SKY)/PHPADU + 
     .     (PERERR*DPOS)**2 + (PKERR*(DPOS-SKYBAR))**2
...
C
 ...
 2195       CONTINUE                          ! End of loops over pixels
            ...
            IF (SKY(I) .GT. 0.) THEN
               K = MIN0(3, MAX0(0, 6-INT(ALOG10(SKY(I)))))
            ELSE
               K = MIN0(3, MAX0(0, 5-INT(ALOG10(-SKY(I)+0.001))))
            END IF
            WRITE(iformat,12) K
 12         FORMAT('(I6, 3F9.3, F9.4, F9.',I2,', F9.0, 2F9.3)')
            WRITE (1,iformat) ID(I), XC(I), YC(I), MAG(I), ERR, SKY(I), 
     .           FLOAT(NITER), CHI(I), SHARP
c  321       FORMAT (I6, 3F9.3, F9.4, F9.<K>, F9.0, 2F9.3)
