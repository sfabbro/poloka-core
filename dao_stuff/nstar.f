     ...
     .     NCOL, NROW, WATCH, FITRAD, E1, E2, GLOBAL_SKY)
C
C=======================================================================
C
...
C
      ...
      LOGICAL SKIP(MAXSTR)
C
      REAL AMIN1, AMAX1, ABS, SQRT, USEPSF, GLOBAL_SKY
      INTEGER MIN0, MAX0
C
      CHARACTER*80 LINE
      CHARACTER*200 COOFIL, MAGFIL, PSFFIL, FITFIL, GRPFIL, SWITCH,
     .     EXTEND
      CHARACTER CASE*4
...
C      IF (GRPFIL .EQ. 'END-OF-FILE') THEN          ! CTRL-Z was entered
C         RETURN
C      END IF
C
...
C  980 CALL GETNAM ('File for results:', FITFIL)
...
C
  ...
  623    FORMAT (3I5, 5X, 'Star', I7, ' merged with star', I7,
...
C
      DPOS=AMAX1(0., DATA(IX,IY)+GLOBAL_SKY-D)
C
...
C
      ...
      SIGSQ=DPOS/PHPADU+RDNOIS+(PERERR*DPOS)**2
     .      +(PKERR*(DPOS-SKYBAR))**2
...
C
 ...
 2560 IF (WATCH .GT. 0.5) THEN
  ...
  624    FORMAT (3I6, 5X, 'Star', I7, ' has disappeared.')
         CALL OVRWRT (LINE(1:51), 3)
      ELSE
         WRITE (6,625) ID(IFAINT)
  625    FORMAT(1X, 'Star', I7, ' has disappeared.')
      END IF
...
C
  ...
  321    FORMAT (I6, 3F9.3, F9.4, F9.3, F9.0, F9.2, F9.3)
      ELSE
         WRITE (1,322) ID(I), XC(I), YC(I), MAG(I), ERR, SKY(I), 
     .     FLOAT(NITER), CHI(I), SHARP(I)
  322    FORMAT (I6, 3F9.3, F9.4, F9.2, F9.0, F9.2, F9.3)
      END IF
 2910 CONTINUE
      WRITE (LINE, 622) NITER, NSTR, NTOT
