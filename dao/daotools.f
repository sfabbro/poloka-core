...
C#######################################################################
C
      SUBROUTINE  APPEND(IFILE1, IFILE2, CMBFILE)
C
C=======================================================================
C
...
C      IFILE1=' '
C      CALL GETNAM ('First input file:', IFILE1)
C      IF (IFILE1 .EQ. 'END-OF-FILE') THEN
C         IFILE1 = ' '
C         RETURN
C      END IF
C
...
C
      IFILE2=' '
C  960 CALL GETNAM ('Second input file:', IFILE2)
C      IF ((IFILE2 .EQ. 'END-OF-FILE') .OR.
C     .     (IFILE2 .EQ. 'GIVE UP')) THEN
C         CALL CLFILE (1)
C         RETURN
C      END IF
...
C         GO TO 960
      ...
      END IF
C
C      IFILE1 = SWITCH(IFILE1, CASE('.cmb'))
C  970 CALL GETNAM ('Output file:', IFILE1)
C      IF ((IFILE1 .EQ. 'END-OF-FILE') .OR.
C     .     (IFILE1 .EQ. 'GIVE UP')) THEN
C         CALL CLFILE (1)
C         CALL CLFILE (2)
C         RETURN
C      END IF
...
C
C      CALL TBLANK                                   ! Type a blank line
C  950 CALL GETNAM ('Input group file:', GRPFIL)
C      IF ((GRPFIL .EQ. 'END-OF-FILE') .OR.
C     .     (GRPFIL .EQ. 'GIVE UP')) THEN
C         GRPFIL = ' '
C         RETURN
C      END IF
C
...
C
C      CALL GETDAT ('Minimum, maximum group size:', SIZE, 2)
C      IF (SIZE(1) .LT. -1.E30) GO TO 9010           ! CTRL-Z was entered
C      MINGRP=NINT(SIZE(1))
C      MAXGRP=NINT(SIZE(2))
C
C Get the name of the output group file and open it.
C
C      MAGFIL=GRPFIL
C  960 CALL GETNAM ('Output group file:', MAGFIL)
...
C#######################################################################
C
      SUBROUTINE  DUMP  (F, NCOL, NROW, COORDS, SIZE)
C
C=======================================================================
C
...
C
C      CALL TBLANK                                   ! Type a blank line
C      CALL GETDAT ('Box size:', SIZE, 1)
C      IF (SIZE .LT. -1.E30) RETURN                  ! CTRL-Z was entered
      NBOX=MAX0(1, MIN0(NINT(SIZE), NSQUARE))
      NHALF=(NBOX-1)/2
C
C 1000 CALL GETDAT ('Coordinates of central pixel:', COORDS, 2)
C
...
C#######################################################################
C
      SUBROUTINE  OFFSET(FILE, DELTA, OFFILE)
C
C=======================================================================
C
...
C
      ...
      INTEGER  ISTAT, IDDEL, NL, NCOL, NROW, ITEMS, NLINE1
      INTEGER NLINE2, ID
      LOGICAL WROTE
C
C-----------------------------------------------------------------------
...
C      IF ((FILE .EQ. 'END-OF-FILE') .OR. (FILE .EQ. 'GIVE UP')) RETURN
      ...
      IF (ISTAT .NE. 0) THEN
C         CALL STUPID ('Error opening input file '//FILE)
C         FILE = 'GIVE UP'
C         GO TO 950
         RETURN
      END IF
...
C         DELTA(I)=0.
C      END DO
C      CALL GETDAT ('Additive offsets ID, DX, DY, DMAG:', DELTA, 4)
C      IF (DELTA(1) .LT. -1.E30) THEN
C         CALL CLFILE (1)
C         RETURN
C      END IF
      IDDEL = NINT(DELTA(1))
