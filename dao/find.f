     ...
     .     MAXSKY, OPT, NOPT, COOFIL, GLOBAL_SKY)
      IMPLICIT NONE
C
C=======================================================================
C
...
C
C                OFFICIAL DAO VERSION:  1996 May 9
C
...
C
      ...
      CHARACTER SWITCH*256, EXTEND*256, LINE*5, CASE*4, ANSWER*1
      ...
      REAL XCEN, YCEN, DX, DY, PHPADU, READNS, SKYMOD, SKYSIG,GLOBAL_SKY
...
C
     ...
     .     READNS, PHPADU, SKYMOD, SKYSIG, IX)
      WRITE (6,610) RELERR
...
C
C 2950 CALL GETNAM ('File for the positions:', COOFIL)
...
D     TYPE *, JX, JY
D     DO 1666 J=1,NBOX
D1666 TYPE 6661, ((D(I,JCYLN(J))), 
D    .     I=MAX0(1,JX-NHALF),MIN0(NCOL,IX+NHALF)),
D    .     ((H(I,JCYLN(J))), I=IX-NHALF,IX+NHALF)
...
C
C ********** IF THE CENTRAL PIXEL IS NEAR THE EDGE OF  ********** 
C **********            THE FRAME, SKIP IT             ********** 
C
      IF ((IX .LT. 1) .OR. (IX .GT. NCOL)) GO TO 3068
         I = MIDDLE + (JX-IX)
         TEMP = 0.0
         DO 3060 J=1,NBOX
            K=JCYLN(J)
...
C
 3190 NSTAR=NSTAR+1
     ...
     .     AMIN1(99.999, AMAX1(-99.999,SHARP)), ROUND
  631 FORMAT(11X, I6, 2F7.1, F9.1, 2F9.2)
      WRITE (3,330) NSTAR, XCEN, YCEN, HEIGHT, 
     .     AMIN1(99.999, AMAX1(-99.999,SHARP)), ROUND, DY
...
C      CALL TBLANK                                    ! Type a blank line
C      CALL GETYN ('Are you happy with this?', ANSWER)
C      IF ((ANSWER .EQ. 'Y') .OR. (ANSWER .EQ. 'E')) GO TO 9000
C      WRITE (6,642) OPT(6), HMIN
C  642 FORMAT(/' Your old threshold was', F8.2, ' sigma =',
C     .     F8.2, ' ADU.'/)
C      CALL GETDAT ('New threshold (in sigmas):', OPT(6), 1)
C      IF (OPT(6) .LT. -1.E30) GO TO 9000             ! CTRL-Z was entered
CC 4030 CALL GETNAM ('Output file name:', COOFIL)
C      IF ((COOFIL .EQ. 'END-OF-FILE') .OR. 
C     .     (COOFIL .EQ. 'GIVE UP')) THEN
C         COOFIL = ' '
C         GO TO 9000
C      END IF
C      COOFIL = EXTEND(COOFIL, CASE('coo'))
C      CALL OUTFIL (3, COOFIL, ISTAT)
C      IF (ISTAT .NE. 0) THEN
C         CALL STUPID ('Error opening output file '//COOFIL)
C         COOFIL = ' '
C         GO TO 9000
C         COOFIL = 'GIVE UP'
C         GO TO 4030
C      END IF
C      HMIN = SQRT(READNS**2 + AMAX1(0.,SKYMOD)/PHPADU)
C      HMIN = 0.01*NINT(100.*OPT(6)*RELERR*HMIN)
C      GO TO 3000
C
C-----------------------------------------------------------------------
C
