...
C
C      CALL TBLANK
C  950 CALL GETNAM ('File with aperture results:', MAGFIL)
...
C
C  900 CALL GETNAM ('File with the PSF:', PSFFIL)
...
C
C      PROFIL=SWITCH(MAGFIL, CASE('.pk'))
C  960 CALL GETNAM ('File for PEAK results:', PROFIL)
...
C
     ...
     .     NX, NY, MAXBOX)
      IF (WATCH .GT. 0.5) WRITE (6,618)
  618 FORMAT (9X, 'STAR', 6X, 'X', 8X, 'Y', 8X, 'MAGNITUDE', 7X)
      IF (WATCH .GT. 0.5) WRITE (6,619) ISTAR, X+LX-1, Y+LY-1, APMAG
  619 FORMAT (8X, I5, 2F9.2, F9.3)
...
C
      WRITE (LINE,621) ISTAR, X, Y
  621 FORMAT (8X, I5, 2F9.3, ' had a singular matrix.')
      CALL STUPID (LINE)
      GO TO 2000
C
...
C
      ...
      REAL SCALE, SKY, RADIUS, HIBAD, BRIGHT, GLOBAL_SKY
...
C
C      FPOS=AMAX1(0., F(IX,IY)-DF)
      FPOS=AMAX1(0., F(IX,IY)+GLOBAL_SKY-DF)
C FPOS = raw data minus residual = model-predicted value of the 
C intensity at this point (which presumably is non-negative).
C
      SIGSQ=FPOS/PHPADU+RONOIS+(PERR*FPOS)**2+
     .     (PKERR*(FPOS-GLOBAL_SKY-SKY))**2
      SIG=SQRT(SIGSQ)
      RELERR=ABS(DF/SIG)
C
...
C
      IF (RHOSQ .LE. 36.) THEN
         ...
         FPOS=AMAX1(0., F(IX,IY)+GLOBAL_SKY-SKY)+SKY+GLOBAL_SKY
