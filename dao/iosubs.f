...
C
 ...
 1001    I=I+1
         IF (I .GT. LEN(FILE)) GO TO 9100
         IF ((FILE(I:I) .NE. ']') .AND. (FILE(I:I) .NE. '>')) GO TO 1001
         GO TO 1000
C added a test for files with . in their names .
      ELSE IF ((FILE(I:I) .EQ. '.').AND.(I .GT. LE-5)) THEN
C      ELSE IF (FILE(I:I) .EQ. '.') THEN
...
C
 ...
 8010    CALL GETDAT ('Lowest good data-value:', LOBAD, 1)
         IF (LOBAD .LE. -1.E38) GO TO 8010
 8020    CALL GETDAT ('Highest good data-value:', HIBAD, 1)
         IF (HIBAD .LE. -1.E38) GO TO 8020
      END IF
      NL=0
      RETURN
C
