...
C
C      CALL TBLANK
C      NEWFIL=SWITCH(FILE, CASE('f'))
C      CALL GETNAM ('Name for output picture:', NEWFIL)
C      IF (NEWFIL .EQ. 'END-OF-FILE') RETURN        ! CTRL-Z was entered
C     CALL COPPIC (NEWFIL, F, NCOL, NROW, ISTAT)
      CALL COPPIC (NEWFIL, ISTAT)
      IF (ISTAT .NE. 0) RETURN                  ! Error creating picture
      CALL TBLANK
C      CALL GETDAT ('Border (pixels):', X, 1)
C      NBORD = MAX0(0, NINT(X))
C      IF (NBORD .GT. 0) THEN
C         CALL GETDAT (
C    .        'Polynomial order (0 = constant, 1 = plane, etc.):',
C    .        X, 1)
C         NPOLY = MAX0(0, MIN0(MAXPOL, NINT(X) ) )
C      END IF
C 1000 CALL TBLANK
c 1005 DATA(2)=-1.
c      CALL GETDAT ('First, last column number:', DATA, 2)
c      IF (DATA(1) .LE. 0.) GO TO 9000
c      IF (DATA(2) .LE. 0.) DATA(2)=DATA(1)
C      LX=MAX0(1, NINT(AMIN1(DATA(1), DATA(2))) )
C      MX=MIN0(NCOL, NINT(AMAX1(DATA(1), DATA(2))) )
C 1010 DATA(2)=-1.
C      CALL GETDAT ('First, last row number:', DATA, 2)
C      IF (DATA(1) .LE. 0.) GO TO 1005
C      IF (DATA(2) .LE. 0.) DATA(2)=DATA(1)
C      LY=MAX0(1, NINT(AMIN1(DATA(1), DATA(2))) )
C      MY=MIN0(NROW, NINT(AMAX1(DATA(1), DATA(2))) )
      IF (NBORD .GT. 0) GO TO 5000
C      CALL GETDAT ('Brightness value:', DATA, 1)
C      IF (DATA(1) .LE. -1.E38) GO TO 1010
      NX = MX-LX+1
      DO I=1,NX
         F(I,1)=BRIGHT
      END DO
