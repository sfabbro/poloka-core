...
class CfhtMegacam: public VirtualInstrument {  /* TYPE_SELECTOR TYPE_TESTER */
...
public:
  ...
  { 
	...
	DeleteWhiteSpaces(StringToUpper(Head.KeyVal("TELESCOP"))) == "CFHT3.6M") return new CfhtMegacam; 
  else return NULL;}
...
bool CfhtMegacam::IsTrimmed(const FitsHeader &Head) const {
...
double CfhtMegacam::AmpGain(const FitsHeader &Head, int Iamp) const
{
  string keyname;
...
Frame CfhtMegacam::extract_frame(const FitsHeader &Head, const char *KeyGenName, int Iamp) const
{
  char keyname[16];
...
Frame CfhtMegacam::OverscanRegion(const FitsHeader &Head, const int Iamp) const
{
...
Frame CfhtMegacam::IlluRegion(const FitsHeader &Head, const int Iamp) const
{
  ...
  if (nx<2048) // one amplifier
      ...
      else
	{
	  ...
	  if(DSECA!="[33:1056,1:4612]") {
	    cerr << "warning in CfhtMegacam::IlluRegion, DSECA is not as expected [33:1056,1:4612]!=" << DSECA<< endl;
	  }
	  string DSECB = Head.KeyVal("DSECB");
	  if(DSECB!="[1057:2080,1:4612]") {
	    cerr << "warning in CfhtMegacam::IlluRegion, DSECB is not as expected [1057:2080,1:4612]!=" << DSECB << endl;
	  }
...
Frame CfhtMegacam::TotalIlluRegion(const FitsHeader &Head) const
{
  if(IsTrimmed(Head))
  ...
  if (Head.HasKey("DATASEC")) {
    ...
    if(datasec=="[0:0,0:0]") {
      ...
      cout << " CfhtMegacam::TotalIlluRegion warning: DATASEC=[0:0,0:0], use instead " << datasec << endl;     
    }
...
Frame CfhtMegacam::AmpRegion(const FitsHeader &Head, const int Iamp) const
{
  int namp = TOADNAMP(Head, false);
  if( namp == 2)
