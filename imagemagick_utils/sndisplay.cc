...
void usage (const char* arg) {
  cerr << "usage: " << arg << " -g <galaxyvignet> -p <psfvignet> -f #flux -o  <gifimage>" << endl;
  exit(EXIT_FAILURE);
}

using namespace std;

void addfitsinmagick(const Image& fitsimage,Magick::Image &magickimage, const Magick::ColorRGB &color, float pixmin, float pixmax) {
  // color 
...
int main(int argc,char **argv) {
  ...
  for(unsigned int i=0;i<nimages;i++) {
    ...
    addfitsinmagick(fits_image,magic_image,colors[i],pixmin,pixmax);
  }
