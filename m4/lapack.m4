# -*- autoconf -*-
# 
# $Id: lapack.m4,v 1.3 2004/02/23 18:14:02 nrl Exp $
# 
...
AC_DEFUN([CHECK_LAPACK],[
 ...
 if test -n "$lapack_headers" ; then
  ...
  LAPACK_CPPFLAGS="-I$LAPACKHOME/src"
 ...
 if test -n "$lapack_libs" ; then
  LAPACK_LDFLAGS="-L$lapack_libs -llapack++ -llapack -lblas  "
 elif test -n "$LAPACKHOME" ; then
  LAPACK_LDFLAGS="-L$LAPACKHOME/Linux-i686 -llapack++ -llapack -lblas"
 elif test -n "$prefix" && test "$prefix" != "NONE" ; then
  LAPACK_LDFLAGS="-L$prefix/lib  -llapack++ -llapack -lblas"
 fi
