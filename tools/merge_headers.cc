#include <fitsio.h>
#include <string>
#include "fitsimage.h" // for FitsFileMode enum */
#include <iostream>
...
{
  cout << prog << " <inputfile> <outputFile>  " << endl;
  cout << " or " << endl;
  cout << prog << " <inputfile1> ... <inputfilen> -d <outputDir> [-c] " << endl;
  cout << " [-c] means rice compression on output " << endl;
  exit(1);
}

static int convert_file(const string &inFile, const string &outFile)
{
...
int main(int nargs, char **args)
{
  ...
  for (int i=1; i< nargs; i++)
	...
	switch (arg[1])
      ...
      else fileNames.push_back(args[i]);
    }
  ...
  if (outDir == "")
    {
      ...
      if (riceCompression)
	return convert_file(fileNames[0], fileNames[1]+"[COMPRESS]");
      else 
	return convert_file(fileNames[0], fileNames[1]);
    }
  ...
  for (unsigned k=0; k< fileNames.size(); ++k)
	...
	outputName= outDir+BaseName(fileNames[k]);	  
      status += convert_file(fileNames[k], outputName);
    }
  return status;
}
